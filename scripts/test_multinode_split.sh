...
#SBATCH --output=/global/scratch/projects/pc_heptheory/jbenabou/NS_megaproject/MeerKAT_data/meerkat_reduction_project/scripts/logs/test_multinode_split_%a.out
#SBATCH --error=/global/scratch/projects/pc_heptheory/jbenabou/NS_megaproject/MeerKAT_data/meerkat_reduction_project/scripts/logs/test_multinode_split_%a.err
#
# Multi-node parallel split test.
# 4 SLURM array tasks, each on a different node,
# each splitting a different 383-channel subband.
#
...
OUT="${TEST_DIR}/multinode_${IDX}.ms"

echo "=== Multi-node split test: task $IDX on $(hostname) ==="
